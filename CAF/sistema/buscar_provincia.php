	...
	<table class="table table-striped table-hover">
	    <thead class="table-info">
	    	<tr>
		        ...
		        <th scope="col">País perteneciente</th>
	    ...
	    <?php
			...
			$sql_registre = mysqli_query($conection, "SELECT COUNT(*) as total_registro FROM provincia WHERE (idprovincia LIKE '%busqueda%' OR nombre_provincia LIKE '%busqueda%' OR pais_id LIKE '%busqueda%') AND estatus = 1 ");
			...
			$query = mysqli_query($conection, "SELECT u.idprovincia, u.nombre_provincia, r.nombrepais FROM provincia u INNER JOIN pais r ON u.pais_id = r.idpais 
								WHERE 
								(u.idprovincia LIKE '%$busqueda%' OR 
								u.nombre_provincia LIKE '%$busqueda%' OR 
								r.nombrepais LIKE '%$busqueda%') 
								AND 
								u.estatus = 1 ORDER BY u.idprovincia ASC LIMIT $desde, $por_pagina");
