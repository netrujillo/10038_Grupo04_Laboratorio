...
<body>
	...
	<table class="table table-striped table-hover">
	    ...
	    <?php
			...
			$sql_registre = mysqli_query($conection, "SELECT COUNT(*) as total_registro FROM pais WHERE (idpais LIKE '%busqueda%' OR nombrepais LIKE '%busqueda%' OR moneda LIKE '%busqueda%' OR plato LIKE '%busqueda%') AND estatus = 1 ");
			...
			$query = mysqli_query($conection, "SELECT idpais, nombrepais, moneda, plato, foto FROM pais 
								WHERE 
								(idpais LIKE '%$busqueda%' OR 
								nombrepais LIKE '%$busqueda%' OR 
								moneda LIKE '%$busqueda%' OR 
								plato LIKE '%$busqueda%') 
								AND 
								estatus = 1 ORDER BY idpais ASC LIMIT $desde, $por_pagina");
